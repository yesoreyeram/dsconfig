...
func TestLoadConfig(t *testing.T) {
	tests := []struct {
		...
		{
			// The default schema example intentionally leaves the Athena
		...
		},
		{
			...
			wantDatabase:   "default",
			wantWorkgroup:  "primary",
			wantOutputLoc:  "s3://example-athena-results/",
		...
		},
		{
			...
			wantDatabase:   "default",
			wantWorkgroup:  "primary",
			wantAssumeARN:  "arn:aws:iam::123456789012:role/GrafanaAthena",
		...
		{
			name: "pascalcase keys are accepted (case-insensitive decode)",
			settings: backend.DataSourceInstanceSettings{
				JSONData: []byte(`{
                    "authType":"default",
		...
		},
		{
			...
			// After ApplyDefaults the auth type is "default", but the Athena
			// selectors are empty, so Validate rejects the config.
			settings: backend.DataSourceInstanceSettings{},
			wantErr:  errors.New("defaultRegion is required"),
		},
		{
			...
			settings: backend.DataSourceInstanceSettings{
				JSONData: []byte(`{"authType":"nonsense","defaultRegion":"us-east-1","catalog":"c","database":"d","workgroup":"w"}`),
			},
		...
		},
		{
			...
			settings: backend.DataSourceInstanceSettings{
				JSONData: []byte(`{"authType":"keys","defaultRegion":"us-east-1","catalog":"c","database":"d","workgroup":"w"}`),
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantCatalog != "" && cfg.Catalog != tt.wantCatalog {
				t.Errorf("Catalog = %q, want %q", cfg.Catalog, tt.wantCatalog)
			}
			if tt.wantDatabase != "" && cfg.Database != tt.wantDatabase {
				t.Errorf("Database = %q, want %q", cfg.Database, tt.wantDatabase)
			}
			if tt.wantWorkgroup != "" && cfg.Workgroup != tt.wantWorkgroup {
				t.Errorf("Workgroup = %q, want %q", cfg.Workgroup, tt.wantWorkgroup)
			}
			if tt.wantOutputLoc != "" && cfg.OutputLocation != tt.wantOutputLoc {
				t.Errorf("OutputLocation = %q, want %q", cfg.OutputLocation, tt.wantOutputLoc)
			}
...
func TestValidate(t *testing.T) {
	...
	// four Athena selectors populated; each case adjusts the auth-related
	// fields to exercise its scenario.
	basic := func() Config {
		return Config{
			DefaultRegion:           "us-east-1",
			Catalog:                 "AwsDataCatalog",
	...
	tests := []struct {
		...
		{
			name: "missing catalog errors",
			mutate: func(c *Config) {
				c.AuthType = AWSAuthTypeDefault
				c.Catalog = ""
			},
			wantErr: "catalog is required",
		},
		{
		...
		{
			name: "missing workgroup errors",
			mutate: func(c *Config) {
				c.AuthType = AWSAuthTypeDefault
				c.Workgroup = ""
			},
			wantErr: "workgroup is required",
		},
	}
