...
func TestLoadConfig(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			example:       "noAuth",
			wantURL:       "http://jaeger.example.com:16686",
			wantNodeGraph: true,
		},
		{
			name:           "basic auth",
			example:        "basicAuth",
			wantURL:        "https://jaeger.example.com",
			wantBasicAuth:  true,
		...
		},
		{
			...
			wantURL:        "https://jaeger.internal.corp",
			wantTLSCA:      true,
			wantSecureKeys: SecureJsonDataConfig{SecureJsonDataKeyTLSCACert},
		},
		{
			name:                  "trace-by-ID time params",
			example:               "traceIdTimeParams",
			wantURL:               "https://jaeger.example.com",
			wantTraceIDTimeParams: true,
		},
		{
		...
		},
		{
			...
			settings: backend.DataSourceInstanceSettings{
				URL:      "https://jaeger",
				JSONData: []byte(`{"tlsAuth":true,"serverName":"jaeger"}`),
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if cfg.TraceIdTimeParams.Enabled != tt.wantTraceIDTimeParams {
				t.Errorf("TraceIdTimeParams.Enabled = %v, want %v", cfg.TraceIdTimeParams.Enabled, tt.wantTraceIDTimeParams)
			}
...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			name:    "missing URL",
			cfg:     Config{},
			wantErr: "Jaeger URL (root.url) is required",
		},
		{
