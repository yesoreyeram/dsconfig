...
// (authType, user, sandbox, tokenUrl) are the jsonData shape; its non-jsonData
// field (DecryptedSecureJSONData) is tagged json:"-" and is skipped by the
// conformance walker. The six secrets live in DecryptedSecureJSONData, matching
// the secureJsonData fields declared in dsconfig.json.
