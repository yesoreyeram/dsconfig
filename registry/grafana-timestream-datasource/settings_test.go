...
func TestLoadConfig(t *testing.T) {
	tests := []struct {
		...
		wantDefDatabase   string
		wantDefTable      string
		wantDefMeasure    string
		...
		},
		{
			...
			wantDefaultReg:    "us-east-1",
			wantProfile:       "my-timestream-profile",
			wantEffectiveProf: "my-timestream-profile",
		},
		{
		...
		},
		{
			...
			wantAuthType:   AWSAuthTypeKeys,
			wantDefaultReg: "us-east-1",
			wantAssumeARN:  "arn:aws:iam::123456789012:role/GrafanaTimestream",
		...
		{
			name:            "with macro defaults",
			example:         "withMacroDefaults",
			wantAuthType:    AWSAuthTypeDefault,
			wantDefaultReg:  "us-east-1",
			wantDefDatabase: "sampleDB",
			wantDefTable:    "IoT",
			wantDefMeasure:  "speed",
		},
		{
		...
		{
			name: "macro defaults survive validation without values",
			// defaultDatabase/defaultTable/defaultMeasure are optional at
			// save-time — LoadConfig should not require them.
			settings: backend.DataSourceInstanceSettings{
				JSONData: []byte(`{
                    "authType":"default",
                    "defaultRegion":"us-east-1",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantDefDatabase != "" && cfg.DefaultDatabase != tt.wantDefDatabase {
				t.Errorf("DefaultDatabase = %q, want %q", cfg.DefaultDatabase, tt.wantDefDatabase)
			}
			if tt.wantDefTable != "" && cfg.DefaultTable != tt.wantDefTable {
				t.Errorf("DefaultTable = %q, want %q", cfg.DefaultTable, tt.wantDefTable)
			}
			if tt.wantDefMeasure != "" && cfg.DefaultMeasure != tt.wantDefMeasure {
				t.Errorf("DefaultMeasure = %q, want %q", cfg.DefaultMeasure, tt.wantDefMeasure)
			}
...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "macro defaults do not affect validation",
			mutate: func(c *Config) {
				c.AuthType = AWSAuthTypeDefault
