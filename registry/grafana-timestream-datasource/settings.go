// Package timestreamdatasource contains the configuration models for the
// Amazon Timestream datasource plugin (`grafana-timestream-datasource`).
//
// The Timestream plugin's editor composes the shared `@grafana/aws-sdk`
// `ConnectionConfig` (rendered without `showHttpProxySettings`,
// `hideAssumeRoleArn`, or `skipEndpoint`, and with a Timestream-specific
// `defaultEndpoint` placeholder) plus a small "Timestream Details" block of
// three ConfigSelect fields for macro defaults: `defaultDatabase`,
// `defaultTable`, and `defaultMeasure`. The plugin's backend `Load`
// (pkg/models/settings.go:24-45) unmarshals jsonData verbatim, then adds two
// legacy fallbacks: `Region → DefaultRegion` when Region is empty/"default",
// and `Profile → config.Database` (top-level datasource `database` field)
// when Profile is empty.
//
// The Config below flattens the upstream `awsds.AWSDatasourceSettings` into
// a single Go value (matching what other AWS registry entries do — see
// `registry/grafana-athena-datasource`, `registry/grafana-x-ray-datasource`)
// and adds a separate `Database` root field to carry the legacy profile
// fallback.
...
const PluginID = "grafana-timestream-datasource"
...
//
// The upstream `awsds.AuthType` is a Go int enum with a custom MarshalJSON /
// UnmarshalJSON that maps to/from the string forms below and treats legacy
// values (`sharedCreds`, `arn`) as their modern equivalents. We keep the
// type as a plain string alias here because the dsconfig registry only
// cares about the on-wire representation.
...
const (
	...
	// grafana-timestream-datasource is in ConnectionConfig's allow-list
...
const (
	...
	// backend (pkg/models/settings.go:42) still reads it from decrypted
	// secure data.
...
// Config is the fully loaded configuration of a Timestream datasource
// instance. It flattens the upstream `DatasourceSettings`
// (`pkg/models/settings.go:12-21`), which embeds
// `awsds.AWSDatasourceSettings` and adds `DefaultDatabase`, `DefaultTable`,
// `DefaultMeasure` (all camelCase json tags matching the frontend).
//
// One root-level datasource field is carried: `Database`, used as a legacy
// fallback for `Profile` when the latter is empty (`pkg/models/settings.go:36-38`).
// It is tagged `json:"-"` so it does not collide with the jsonData decode;
// callers must populate it from `backend.DataSourceInstanceSettings.Database`
// before invoking Validate, which LoadConfig does.
type Config struct {
	...
	// this into `Region` when Region is empty; see pkg/models/settings.go:32-34).
	DefaultRegion string `json:"defaultRegion,omitempty"`

	// ---- Timestream-specific ----

	// DefaultDatabase seeds the {{database}} macro in queries.
	DefaultDatabase string `json:"defaultDatabase,omitempty"`
	// DefaultTable seeds the {{table}} macro in queries. Depends on
	// DefaultDatabase for the editor's ConfigSelect fetch.
	DefaultTable string `json:"defaultTable,omitempty"`
	// DefaultMeasure seeds the {{measure}} macro in queries. Depends on
	// DefaultDatabase + DefaultTable for the editor's ConfigSelect fetch.
	DefaultMeasure string `json:"defaultMeasure,omitempty"`

	// ---- Root-level datasource field ----

	// Database is the top-level datasource `database` field. The Timestream
	// backend uses it as a legacy fallback for Profile
	// (`pkg/models/settings.go:36-38`) when Profile is empty. Tagged
	// `json:"-"` so it does not collide with a `database` key inside
	// jsonData; LoadConfig populates it from `settings.Database`.
...
// mirroring the fallback in `pkg/models/settings.go:36-38`: if
...
// mirrors the plugin's backend Load flow: unmarshal jsonData, capture the
// root-level `Database` legacy-profile fallback, copy the decrypted secrets
// used by the plugin, apply the editor's parity defaults (curated), then
// Validate the runtime contract. The three phases are documented
// individually below.
//
...
// `DatasourceSettings.Load` to sync to. Callers that need each phase
// individually can invoke ApplyDefaults and Validate directly on the
// returned Config.
func LoadConfig(ctx context.Context, settings backend.DataSourceInstanceSettings) (Config, error) {
	...
	logger.Debug("loading timestream datasource config")
	...
	// Capture the legacy root-level `database` value that the plugin's Load
	// uses as the profile fallback.
	...
	if err := cfg.Validate(); err != nil {
		logger.Error("timestream datasource config failed validation", "err", err)
		return cfg, err
	}

	logger.Debug("timestream datasource config loaded",
		"authType", cfg.AuthType,
		"defaultRegion", cfg.DefaultRegion,
		"hasProfile", cfg.Profile != "",
		"hasLegacyProfileFallback", cfg.Profile == "" && cfg.Database != "",
		"hasDefaultDatabase", cfg.DefaultDatabase != "",
		"hasDefaultTable", cfg.DefaultTable != "",
		"hasDefaultMeasure", cfg.DefaultMeasure != "",
	)
	return cfg, nil
}
...
//
// DefaultRegion, DefaultDatabase, DefaultTable, and DefaultMeasure
// intentionally have no default because they must be picked from the
...
// present, and the AWS region the backend actually needs to build service
// clients (`defaultRegion`) is non-empty. The Timestream macro defaults
// (defaultDatabase/defaultTable/defaultMeasure) are NOT validated — the
// editor does not gate save on them and queries can supply their own
// database/table/measure per request. Errors are joined so callers see
