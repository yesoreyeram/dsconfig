...
//
// The Config struct itself is the settings model: its json-tagged field
// (auth.id, via the nested AuthConfig — the schema declares "auth" as a section)
// is the jsonData shape; its non-jsonData field (DecryptedSecureJSONData) is
// tagged json:"-" and is skipped by the conformance walker.
