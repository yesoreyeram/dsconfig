...
func TestLoadConfig(t *testing.T) {
	t.Run("default example fails validation (empty token)", func(t *testing.T) {
		if _, err := LoadConfig(t.Context(), backend.DataSourceInstanceSettings{
			JSONData: []byte(`{"services":{"Netlify":{"auth":{"id":"bearer_token"}}}}`),
	...
	t.Run("valid config loads and defaults auth id", func(t *testing.T) {
		cfg, err := LoadConfig(t.Context(), backend.DataSourceInstanceSettings{
			DecryptedSecureJSONData: map[string]string{"Netlify.token": "tok"},
		})
		if err != nil {
			t.Fatalf("LoadConfig: %v", err)
		}
		if cfg.Services.Netlify.Auth.Id != AuthMethodBearerToken {
			t.Errorf("auth.id = %q, want %q", cfg.Services.Netlify.Auth.Id, AuthMethodBearerToken)
		}
...
func TestValidate(t *testing.T) {
	ok := Config{
		Services:                ServicesConfig{Netlify: ServiceConfig{Auth: AuthConfig{Id: AuthMethodBearerToken}}},
		DecryptedSecureJSONData: map[SecureJsonDataKey]string{SecureJsonDataKeyToken: "tok"},
	}
	if err := ok.Validate(); err != nil {
		t.Fatalf("Validate ok: %v", err)
	}
	if err := (Config{Services: ServicesConfig{Netlify: ServiceConfig{Auth: AuthConfig{Id: "bogus"}}}}).Validate(); err == nil || !strings.Contains(err.Error(), "unknown auth method") {
