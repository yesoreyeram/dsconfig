// Package netlifydatasource contains the configuration models for the Netlify
// datasource plugin (grafana-netlify-datasource) from the grafana/plugins
...
const PluginID = "grafana-netlify-datasource"

// ServiceID is the single service id declared by the Netlify spec (capitalized "Netlify").
const ServiceID = "Netlify"
...
const (
	// AuthMethodBearerToken is the only auth method the Netlify spec exposes
...
const (
	// SecureJsonDataKeyToken is the Netlify personal access token (bearer) for
	// the "Netlify" service. The SDK reads it as DecryptedSecureJSONData["Netlify.token"].
	SecureJsonDataKeyToken SecureJsonDataKey = "Netlify.token"
)
...
// Config is the fully loaded configuration of a Netlify datasource instance: a
...
type Config struct {
	...
	// DecryptedSecureJSONData holds the decrypted secure values by key (Netlify.token).
	DecryptedSecureJSONData map[SecureJsonDataKey]string `json:"-"`
}

// ServicesConfig holds the per-service configuration keyed by service id.
type ServicesConfig struct {
	Netlify ServiceConfig `json:"Netlify"`
}

// ServiceConfig is the configuration for the "Netlify" service.
type ServiceConfig struct {
	Auth AuthConfig `json:"auth"`
}

// AuthConfig is the auth block stored at jsonData.services.Netlify.auth.
...
// validated Config: parse jsonData + copy the decrypted secret (Netlify.token),
...
func LoadConfig(ctx context.Context, settings backend.DataSourceInstanceSettings) (Config, error) {
	...
	logger.Debug("loading netlify datasource config")
	...
	if err := cfg.Validate(); err != nil {
		logger.Error("netlify datasource config failed validation", "err", err)
		return cfg, err
	}

	logger.Debug("netlify datasource config loaded", "authMethod", cfg.Services.Netlify.Auth.Id)
	return cfg, nil
}
...
func (c *Config) ApplyDefaults() {
	if c.Services.Netlify.Auth.Id == "" {
		c.Services.Netlify.Auth.Id = AuthMethodBearerToken
	}
}
...
func (c Config) Validate() error {
	...
	switch c.Services.Netlify.Auth.Id {
	case AuthMethodBearerToken:
		if c.DecryptedSecureJSONData[SecureJsonDataKeyToken] == "" {
			errs = append(errs, errors.New(`personal access token (secureJsonData "Netlify.token") is required`))
		}
	case "":
		errs = append(errs, errors.New("auth method is required"))
	default:
		errs = append(errs, fmt.Errorf("unknown auth method %q", c.Services.Netlify.Auth.Id))
	}
