/**
 * Configuration models for the Cloudflare datasource plugin (`grafana-cloudflare-datasource`) from
 ...
 * - `plugins/grafana-cloudflare-datasource/src/spec.ts` — one service (`cloudflare`), one server
 *   (`client_api`, https://api.cloudflare.com/client/v4, no variables), and one bearer auth method
 *   (`bearer_token`).
...
/** Auth block stored at `jsonData.services.cloudflare.auth`. The bearer token itself is a secret. */
export type CloudflareAuthConfig = {
  /** Selected auth method id; `bearer_token` (bearer). The backend defaults `auth.id` to it. */
  id?: 'bearer_token';
};

/** Per-service config stored at `jsonData.services.cloudflare`. */
export type CloudflareServiceConfig = {
...
export type JsonDataConfig = {
  services?: {
    cloudflare?: CloudflareServiceConfig;
  };
};
 ...
 * `secureJsonFields`):
 * - `cloudflare.token` — the Cloudflare API token (bearer) for the `cloudflare` service.
 */
export type SecureJsonDataConfig = Array<'cloudflare.token'>;
