...
// truth for the X-Ray / AWS Application Signals datasource configuration.
//
//go:embed dsconfig.json
var configSchemaJSON []byte

// ConfigSchema parses, resolves, and returns the declarative dsconfig schema
// (single source of truth) for the X-Ray datasource.
func ConfigSchema() (*dsconfig.Schema, error) {
	return dsconfig.ParseAndResolveSchemaJSON(configSchemaJSON)
}

// NewSchema returns the full k8s-style SDK plugin schema for the X-Ray
...
// SettingsExamples returns k8s-style example configurations for the X-Ray
// datasource, covering the default configuration and each editor-selectable
// AWS authentication provider plus an STS assume-role variant and the legacy
// `arn` value. Each example is a full instance-settings object with the
		...
		Examples: map[string]*spec3.Example{
			...
			"grafanaAssumeRole": {
				ExampleProps: spec3.ExampleProps{
					...
					Description: "Grafana Cloud's temporary-credentials broker. Feature-gated on the awsDatasourcesTempCredentials toggle; the editor hides externalId/endpoint when this provider is selected because Grafana derives them itself. Plugin v2.17.0 added sessionToken handling to support this flow.",
					Value: map[string]any{
						"jsonData": map[string]any{
			...
			"assumeRoleFromKeys": {
				ExampleProps: spec3.ExampleProps{
					...
					Value: map[string]any{
						"jsonData": map[string]any{
							"authType":      string(AWSAuthTypeKeys),
							"assumeRoleArn": "arn:aws:iam::123456789012:role/GrafanaXRay",
