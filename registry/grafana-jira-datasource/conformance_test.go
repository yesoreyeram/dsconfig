...
// (url, user, hosting, scopedToken, cloudId, authMethod, oauthClientID) are the
// jsonData shape; its non-jsonData field (DecryptedSecureJSONData) is tagged
// json:"-" and is skipped by the conformance walker. The two secrets live in
// DecryptedSecureJSONData, matching the secureJsonData fields declared in
// dsconfig.json (token, oauthClientSecret).
